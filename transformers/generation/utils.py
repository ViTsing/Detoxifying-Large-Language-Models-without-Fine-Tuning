...
class SampleDecoderOnlyOutput(ModelOutput):
    ...
    attentions: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    hallucination_layer_dist: Optional[Dict[int, int]] = None
    safe_layer_dist: Optional[Dict[int, int]] = None
...
class GenerationMixin:
    ...
    def generate(
        ...
        ddsc_decoding: Optional[bool] = None,
        ...
        **kwargs,
        ...
        elif is_sample_gen_mode and ddsc_decoding:
        ...
        **model_kwargs,
        ...
        with torch.no_grad():
            while True:
                ...
                #     early_exit_layers = [base_layer, end_layer]
                #     num_base_layers = 1
                #     hallucination_layer_dist = {}
                #     safe_layer_dist = {}

                if(candidate_layers is not None):
                    early_exit_layers = candidate_layers + [end_layer]
                    num_base_layers = len(candidate_layers)
                    hallucination_layer_dist = {l:0 for l in candidate_layers}
                    safe_layer_dist = {l:0 for l in early_exit_layers}
                ...
                if base_layer is not None:
                    hallucination_logits = dict_outputs[base_layer]
                    final_logits = dict_outputs[end_layer]
                    if relative_top > 0.0:
                        ...
                        hallucination_logits[0][mask] = -1e3
                    logits = final_logits - hallucination_logits
                    ...
                    stacked_hallucination_layers = all_hidden_states[:-1, :, :]  # 选择除最后一层之外的所有层

                    # 3. 获取早期退出层的隐藏状态
                    stacked_safe_layers = all_hidden_states[early_exit_layers, :, :]  # 选择早期退出层

                    # 2. Calculate the softmax values for end_layer and all hallucination_layers
                    softmax_end_layer = F.softmax(dict_outputs[end_layer][:, -1, :], dim=-1)  # shape: (batch_size, num_features)
                    ...
                    softmax_safe_layers = F.softmax(stacked_safe_layers, dim=-1)  # shape: (num_hallucination_layers, batch_size, num_features)

                    # 3. Calculate M, the average distribution
                    M = 0.5 * (softmax_end_layer[None, :, :] + softmax_hallucination_layers)  # shape: (num_hallucination_layers, batch_size, num_features)

                    # N = 0.5 * (softmax_toxic_layer[None, :, :] + softmax_safe_layers)  # shape: (num_hallucination_layers, batch_size, num_features)
                    N_1 = 0.5 * (softmax_toxic_max_layer[None, :, :] + softmax_safe_layers)  # shape: (num_hallucination_layers, batch_size, num_features)

                    # 4. Calculate log-softmax for the KL divergence
                    log_softmax_end_layer = torch.log(softmax_end_layer)
                    ...
                    log_softmax_safe_layers = F.log_softmax(stacked_safe_layers, dim=-1)  # shape: (num_hallucination_layers, batch_size, num_features)
                    ...
                    kl3_1 = F.kl_div(log_softmax_toxic_max_layer[None, :, :], N_1, reduction='none').mean(-1)  # shape: (num_hallucination_layers, batch_size)
                    kl4_1 = F.kl_div(log_softmax_safe_layers, N_1, reduction='none').mean(-1)  # shape: (num_hallucination_layers, batch_size)
                    js_safe_max_divs = 0.5 * (kl3_1 + kl4_1)  # shape: (num_hallucination_layers, batch_size)
                    # 6. Reduce the batchmean
                    js_divs = js_divs.mean(-1)  # shape: (num_hallucination_layers,)
                    hallucination_layer = candidate_layers[int(js_divs.argmax().cpu().item())]
                    hallucination_layer_dist[hallucination_layer] += 1

                    js_divs_safe_max_mean = js_safe_max_divs.mean(-1)  # shape: (num_hallucination_layers,)
                    js_divs_safe_max_mean[0] = float('-inf')
                    top_two_indices_max = js_divs_safe_max_mean.topk(2).indices.cpu().tolist()
                    safe_layer = early_exit_layers[top_two_indices_max[0]]
                    # print("safe_layer:{}".format(safe_layer))
                    # print("safe_layer:{}".format(safe_layer))
                    # safe_layer_dist[safe_layer] += 1
                    # safe_layer = candidate_layers[int(js_safe_divs.argmax().cpu().item())]
                    # safe_layer_dist[safe_layer] += 1


                    # safe_logits = dict_outputs[safe_layer][:, -1, :]
                    ...
                    hallucination_logits = dict_outputs[hallucination_layer][:, -1, :]
                    final_logits = dict_outputs[end_layer][:, -1, :]
                    # if relative_top > 0.0:
                    ...
                    if relative_top > 0.0:
                        ...
                        # bad_logits = toxic_max_logits+safe_max_logits
                        b_logits = hallucination_logits-safe_max_logits+toxic_max_logits
                        final_logits = torch.where(relative_top_mask, -float("Inf"), final_logits)
                        # bad_logits = torch.where(relative_top_mask, -1e3, bad_logits)
                        b_logits = torch.where(relative_top_mask, -1e3, b_logits)
                        # hallucination_logits = torch.where(relative_top_mask, -1e3, hallucination_logits)
                        ...
                        # bad_logits = hallucination_logits-safe_logits

                        final_logits = final_logits.log_softmax(dim=-1)
                        # good_logits = good_logits.log_softmax(dim=-1)
                        # bad_logits = bad_logits.log_softmax(dim=-1)
                        b_logits = b_logits.log_softmax(dim=-1)
                        # hallucination_logits = hallucination_logits.log_softmax(dim=-1)
                        # toxic_logits = toxic_logits.log_softmax(dim=-1)


                    # logits = final_logits - hallucination_logits-toxic_logits
                    # print("bad_logits:{}", bad_logits)

                    # logits = final_logits - hallucination_logits

                    logits = final_logits - b_logits
        ...
        #         # print("safe_layer_dist:{}", safe_layer_dist)
        #         # print("hallucination_layer_dist:{}", hallucination_layer_dist)
            ...
            if return_dict_in_generate:
                ...
                if self.config.is_encoder_decoder:
                    ...
                    return SampleDecoderOnlyOutput(
                        ...
                        attentions=decoder_attentions,
                        hidden_states=decoder_hidden_states,
                        hallucination_layer_dist=hallucination_layer_dist,
                        safe_layer_dist=safe_layer_dist,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if(ddsc==True):
                ...
                # 1. Gets the hidden state of all candidate_layers
                stacked_hallucination_layers = torch.stack([dict_outputs[i][:, -1, :] for i in candidate_layers], dim=0)
                
                stacked_safe_layers = torch.stack([dict_outputs[i][:, -1, :] for i in candidate_layers], dim=0)
                ...
                # 3. Calculate the softmax values for safe_layers and toxic_layers
                ...
                N = 0.5 * (softmax_toxic_max_layer[None, :, :] + softmax_safe_layers)  # shape: (num_hallucination_layers, batch_size, num_features)

                # 4. Calculate log-softmax for the KL divergence
                log_softmax_end_layer = torch.log(softmax_end_layer)# shape: (num_end_layer, batch_size, num_features)
                
                log_softmax_hallucination_layers = F.log_softmax(stacked_hallucination_layers, dim=-1)  # shape: (num_hallucination_layers, batch_size, num_features)
                
                log_softmax_toxic_max_layer = torch.log(softmax_toxic_max_layer)# shape: (num_toxic_max_layer, batch_size, num_features)

                log_softmax_safe_layers = F.log_softmax(stacked_safe_layers, dim=-1)  # shape: (num_safe_layers, batch_size, num_features)
                # 5. Calculate the KL divergences and then the JS divergences
                kl1 = F.kl_div(log_softmax_end_layer[None, :, :], M, reduction='none').mean(-1)  # shape: (num_end_layer, batch_size)
                
                kl2 = F.kl_div(log_softmax_hallucination_layers, M, reduction='none').mean(-1)  # shape: (num_hallucination_layers, batch_size)
                
                js_divs = 0.5 * (kl1 + kl2)  

                kl3_1 = F.kl_div(log_softmax_toxic_max_layer[None, :, :], N, reduction='none').mean(-1)  # shape: (num_toxic_max_layer, batch_size)
                
                kl4_1 = F.kl_div(log_softmax_safe_layers, N, reduction='none').mean(-1)  # shape: (num_safe_layers, batch_size)
                ...
                js_divs_safe_max_mean = js_safe_max_divs.mean(-1)  # shape: (num_safe_layers,)
                js_divs_safe_max_mean[0] = float('-inf')
                top_two_indices_max = js_divs_safe_max_mean.topk(2).indices.cpu().tolist()
                safe_layer = early_exit_layers[top_two_indices_max[0]]
                safe_layer_dist[safe_layer] += 1
                # 7. Compute the distribution of layers
                ...
                if relative_top > 0.0:
                    ...
                    relative_top_mask = self.get_relative_top_filter(final_logits, relative_top)#
                    b_logits = hallucination_logits+toxic_max_logits-safe_max_logits
                    final_logits = torch.where(relative_top_mask, -float("Inf"), final_logits)
                    b_logits = torch.where(relative_top_mask, -1e3, b_logits)
                    final_logits = final_logits.log_softmax(dim=-1)
                    b_logits = b_logits.log_softmax(dim=-1)
                #9.Dynamic Safety Constraint
                logits = final_logits-b_logits
                next_token_logits = logits
                toxic = toxic_layer[0]
            else:
        ...
        if return_dict_in_generate:
            ...
            return input_ids,safe_layer,toxic
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        #     # # 1. Stacking all hallucination_layers into a new dimension
        #     # stacked_hallucination_layers = torch.stack([dict_outputs[i][:, -1, :] for i in candidate_layers], dim=0)
        #     # stacked_safe_layers = torch.stack([dict_outputs[i][:, -1, :] for i in early_exit_layers], dim=0)#将候选过早层所有的隐藏状态叠加到第0维
        ...
        #     # # 4. Calculate log-softmax for the KL divergence
        #     # log_softmax_end_layer = F.log_softmax(dict_outputs[end_layer][:, -1, :], dim=-1)  # shape: (batch_size, num_features)
        #     # log_softmax_hallucination_layers = F.log_softmax(stacked_hallucination_layers, dim=-1)  # shape: (num_hallucination_layers, batch_size, num_features)
        ...
        #     # kl3 = F.kl_div(log_softmax_toxic_layer[None, :, :], N, reduction='none').mean(-1)  # shape: (num_hallucination_layers, batch_size)
        #     # kl4 = F.kl_div(log_softmax_safe_layers, N, reduction='none').mean(-1)  # shape: (num_hallucination_layers, batch_size)
        #     # js_safe_divs = 0.5 * (kl3 + kl4)  # shape: (num_hallucination_layers, batch_size)
        #     # # 6. Reduce the batchmean
        #     # js_divs = js_divs.mean(-1)  # shape: (num_hallucination_layers,)
        #     # hallucination_layer = candidate_layers[int(js_divs.argmax().cpu().item())]
        #     # hallucination_layer_dist[hallucination_layer] += 1

        #     # js_divs_safe_mean = js_safe_divs.mean(-1)  # shape: (num_hallucination_layers,)
        #     # js_divs_safe_mean[0] = float('-inf')
        #     # top_two_indices = js_divs_safe_mean.topk(2).indices.cpu().tolist()
        #     # safe_layer = early_exit_layers[top_two_indices[0]]
        #     # # print("safe_layer:{}".format(safe_layer))
        #     # # print("safe_layer:{}".format(safe_layer))
        #     # # safe_layer_dist[safe_layer] += 1
        #     # # safe_layer = candidate_layers[int(js_safe_divs.argmax().cpu().item())]
