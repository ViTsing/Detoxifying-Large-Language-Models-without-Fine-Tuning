...
def load_json(file_path):
    try:
        with open(file_path, 'r') as file:
            data = json.load(file)
            # print("Loaded data:", data)  # Print to debug
    ...
    for item in data:
        # print("Processing item:", item)  # Print each item for debugging
        data_dict = dict(
            prefix=item.get(prefix_type, ''),  
            # completion=item.get('completion', ''),#原代码
            completion=item.get('safe generation', ''),#修改
            contradiction_0=item.get('unsafe generation', ''),
            question = item.get('question', ''),
            adversarial_prompt = item.get('adversarial prompt', ''),
            # contradiction_1=item.get('contradiction_1', ''),
            # contradiction_2=item.get('contradiction_2', ''),
        )
        # print("context："+str(item.get('context', '')))
...
# ).to(f"cuda:{hparams.device}")
        with torch.no_grad():
            # input['input_ids'] = input['input_ids'].to(torch.float16)
            # input['input_ids'] = input['input_ids'].long()

            # input_ids = input_ids.long()  # 将 input_ids 转换为 Long 类型
        ...
        for j in range(len(requests)):
            ...
            for layer_index in range(1, len(hidden_states)):
                num_tokens = hidden_states[layer_index].size(0)
                # print(f"Layer {layer_index} has {num_tokens} tokens.")  
                euclidean_distance = torch.dist(hidden_states[layer_index][j * 2], hidden_states[layer_index][j * 2 + 1], p=2)#第几层的第几个token的隐藏状态

                if euclidean_distance.item() > max_distance_value:
                    max_distance_value = euclidean_distance.item()
                    max_distance_layer = layer_index
            toxic_layer.append(max_distance_layer-1)#隐藏层差异最大的层
        return toxic_layer
